...
LevelTest::LevelTest()
{
	...
	Creature* ptr_wall_1 = ptr_initial_core->AddCreature(cre_flying_box, &wall_1_position, force);
	Coordinates other_guy_position = { 50,300 };
	Creature* ptr_other_guy = ptr_initial_core->AddCreature(cre_clawy, &other_guy_position, force);
	ptr_other_guy->SetBehaviorMode(beh_run_along_predefined_path);
	Coordinates other_guy2_position = { 500,300 };
	Creature* ptr_other_guy2 = ptr_initial_core->AddCreature(cre_clawy, &other_guy2_position, force);
	...
	//Add trigger layer!
	//#TODO - uproœciæ, by warstwa na triggery by³a tworzona domyœlnie
	LevelComponent* ptr_trigger_layer =  ptr_components_factory->SpawnLevelComponent(levco_triggers);

	//Trigger to win level
	std::string signal_to_win = "winning";
	PreciseRect event_area = {100,100,100,100};
	Creature* ptr_winning_trigger = ptr_trigger_layer->AddCreature(cre_event_trigger, &event_area, merge, signal_to_win);
